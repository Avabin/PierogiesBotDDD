...
public class EventDispatcherHostedService : IHostedService
{
    ...
    public EventDispatcherHostedService(IMessageBroker messageBroker, ICommandHandlerFactory commandHandlerFactory,
...
#pragma warning restore CS4014
    ...
    private async Task HandleQuery(IQuery query, Guid? correlationId, string replyTo)
    {
        ...
        _logger.LogTrace("Sending query result {CorrelationId} {ResultType} to {Target}", correlationId,
                         result.GetType().Name, replyTo);
        await _messageBroker.SendToQueueAsync(result, replyTo, correlationId);
    }

    private async Task HandleCommand(ICommand command, Delivery? context = null)
    {
