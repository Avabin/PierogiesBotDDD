...
internal class MongoRepository<T> : IRepository<T> where T : Entity
{
    ...
    public MongoRepository(IMongoClient client, IOptions<MongoSettings> options, string collectionName, ILogger<MongoRepository<T>> logger)
    {
    ...
    {
        _logger.LogTrace("Finding entity {EntityType} by field {Field} with value {Value}", typeof(T).Name, field, value);
    ...
    {
        _logger.LogTrace("Adding domain event {EventType} to entity {EntityType} with id {Id}", @event.GetType(), typeof(T).Name, id);
    ...
    {
        _logger.LogTrace("Removing domain event {EventType} from entity {EntityType} with id {Id}", @event.GetType(), typeof(T).Name, id);
        var entity = await FindByIdAsync(id);
        if (entity is null) return;
