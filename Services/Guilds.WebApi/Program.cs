...
app.MapPost("/{id:long}", async ([FromBody] GuildDto guildDto,[FromServices] IMessageBroker broker, [FromRoute] ulong id) =>
{
...
app.MapPost("/{id:long}/channels", async ([FromBody] SubscribedChannelView channel, [FromRoute] ulong id, [FromServices] IMessageBroker broker) =>
{
...
app.MapDelete("/{id:long}/channels/{channelId:long}", async ([FromRoute] ulong id, [FromRoute] ulong channelId, [FromServices] IMessageBroker broker) =>
{
    var notificationObservable = broker.GetNotificationsObservable<UnsubscribedFromChannel>(id.ToString());
